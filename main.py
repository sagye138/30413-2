...
if not st.session_state.alive:
    ...
    # 엄마 고르기 / 엄마 폭발 이벤트
        ...
        st.button("재벌가 마라탕집 사장님 엄마", on_click=select_mom, args=('rich',))
        st.button("무술 고수 대륙의 어머니", on_click=select_mom, args=('fighter',))
        st.button("평범하고 인자한 시골 어머니", on_click=select_mom, args=('gentle',))
    ...
    # 루트들
        ...
        st.button("여유롭게 건넌다", on_click=do_action, args=(None, True, "파란불에 여유롭게 걷다가 배달 오토바이에 치여서 납치당해 죽었습니다!", "무법지대"))
        ...
        st.button("요즘 유행하는 힙합 랩 시전", on_click=do_action, args=(None, True, "탁원이 탁원이 (RED RED) 중국인 중국인 (RED RED) 워쓰어중궈러 죽었습니다!", "쇼미더대륙"))
